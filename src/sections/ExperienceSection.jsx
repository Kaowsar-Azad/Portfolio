...
function ExperienceCard({ exp, index }) {
  ...
  return (
    <motion.div 
      ...
      className={`relative flex flex-col md:flex-row gap-8 mb-32 ${index % 2 === 0 ? "md:flex-row-reverse" : ""} group z-10 perspective-1000 ${isInView ? "active-card" : ""}`}
    >
      {/* সেন্টার আইকন - গ্লোয়িং ইফেক্ট সহ */}
      <div className="absolute left-6 md:left-1/2 w-12 h-12 rounded-full bg-[#030712] border border-white/10 md:-translate-x-6 flex items-center justify-center text-xl z-20 transition-all duration-500 group-hover:scale-110 group-hover:border-cyan-400/50 shadow-[0_0_20px_rgba(34,211,238,0.1)] group-hover:shadow-[0_0_30px_rgba(34,211,238,0.4)]">
         <div className="absolute inset-0 rounded-full bg-cyan-400/5 animate-pulse" />
         {exp.icon}
      </div>

      <div className="w-full md:w-1/2 pl-24 md:pl-0 z-10">
        <div className="relative p-[1px] rounded-[2.5rem] overflow-hidden transition-all duration-500 shadow-[0_0_20px_rgba(34,211,238,0)] group-[.active-card]:shadow-[0_0_50px_rgba(34,211,238,0.3)]">
          ...
          <div className="relative h-full bg-[#050914]/90 backdrop-blur-3xl p-10 rounded-[2.5rem] border border-white/5 transition-all duration-500 group-[.active-card]:bg-[#0a1128]">
            ...
            <motion.h3 variants={itemVariants} className="text-3xl md:text-4xl font-black mb-3 text-white leading-tight">
              {exp.role}
            </motion.h3>
            
            <motion.p variants={itemVariants} className="text-purple-400 font-semibold mb-6 tracking-widest text-sm uppercase">
              {exp.company}
            </motion.p>
            
            <motion.p variants={itemVariants} className="text-slate-400 text-base md:text-lg leading-relaxed group-hover:text-slate-300 transition-colors">
              {exp.desc}
            </motion.p>
          </div>
        </div>
    ...
    <section id="experience" ref={containerRef} className="pt-10 pb-32 relative bg-[#030712] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-black tracking-tighter bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-text text-transparent">
            EXPERIENCE
          </h2>
        </motion.div>
